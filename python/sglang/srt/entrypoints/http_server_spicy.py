...
SPICY_DEFAULT_CLI_ARGS: Tuple[str, ...] = (
    "--model-path",
    "/mnt/model_garden",
    "--chat-template",
    "/mnt/model_garden/spicy_chat_template.jinja",
    "--quantization",
    "w4afp8",
    "--page-size",
    "1",
...
def get_spicy_default_cli_args() -> List[str]:
    """Return the default CLI arguments for the spicy deployment."""
    return list(SPICY_DEFAULT_CLI_ARGS)


def prepare_spicy_server_args(
    cli_overrides: Optional[Sequence[str]] = None,
) -> ServerArgs:
    """Merge spicy defaults with optional overrides and parse into ServerArgs."""
    argv = list(SPICY_DEFAULT_CLI_ARGS)
    if cli_overrides:
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    """Launch the spicy server with baked-in defaults and optional overrides."""
    cli_overrides = list(argv) if argv is not None else sys.argv[1:]
    server_args = prepare_spicy_server_args(cli_overrides)
    try:
