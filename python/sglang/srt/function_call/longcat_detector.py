...
class LongCatDetector(BaseFormatDetector):
    """
    Detector for LongCat Flash Chat model function call format.
    Format Structure:
    ```
    <longcat_tool_call>
    ...
    ```
    Multiple consecutive calls (no separator):
    ```
    <longcat_tool_call>
    ...
    ```
    Key Components:
    - Tool Call Tags: `<longcat_tool_call>` and `</longcat_tool_call>` wrap each individual call
    - Function Call Object: JSON object with "name" and "arguments" fields
    - No separator between multiple tool calls (they are placed consecutively)
    """
    ...
    def has_tool_call(self, text: str) -> bool:
        """Check if the text contains a LongCat format tool call."""
    ...
    def detect_and_parse(self, text: str, tools: List[Tool]) -> StreamingParseResult:
        ...
        # Find all <longcat_tool_call>\n...\n</longcat_tool_call> blocks
    ...
    def parse_streaming_increment(
        ...
        """
        Streaming incremental parsing for LongCat tool calls.
